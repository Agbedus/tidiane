...
const I18N = {
  en: {
    ...
    hero: { eyebrow: "President, Council of the African Space Agency", name_line1: "H.E. Dr. Tidiane", name_line2: "Ouattara", title: "Steering Africa\u2019s sovereign presence in the space domain", bio: "H.E. Dr. Tidiane Ouattara is a distinguished African space leader, strategist, and international advocate for science, technology, and innovation. He is the elected President of the Council of the African Space Agency and has held senior leadership roles at the Canadian Space Agency and the African Union Commission.", btn_profile: "Profile & Career \u2192", btn_books: "Books & Research" },
    about: { label: "About", heading: "Architect of Africa\u2019s Space Sovereignty", p1: "H.E. Dr. Tidiane Ouattara is the elected President of the Council of the African Space Agency. A space scientist with a PhD in Remote Sensing and GIS from Universit\u00e9 de Sherbrooke (Canada), he has built a 29-year career at the intersection of space science, policy, and international diplomacy \u2014 holding senior leadership roles at the Canadian Space Agency, Natural Resources Canada, Environment and Climate Change Canada, and the African Union Commission.", p2: "He has mobilised over US $155M for African space and Earth observation programmes, led the GMES & Africa initiative across 45 countries, and driven the ratification of the African Space Policy and Strategy by all 55 AU member states. His work bridges science and diplomacy, forging international partnerships that translate space knowledge into agricultural resilience, disaster preparedness, and climate action for African communities.", quote: "Space is not a luxury for Africa. It is infrastructure \u2014 as essential as roads, as vital as water.", stat_55: "Member states represented", stat_30: "Years of Leadership in the Space Sector", stat_100: "International Partnerships", stat_7: "Books" },
...
function renderBooksGrid(books, lang) {
  ...
  grid.innerHTML = books.map((b, i) => {
    ...
    const desc = lang === 'fr' ? b.description_fr : b.description_en;
    ...
    return '<div class="book-card reveal" onclick="openSheet(\'author\')" style="transition-delay:' + delay + 's;">' +
      '<div class="' + coverClass + '">' +
        ...
        '<div class="book-cover-inner">' +
          ...
          '<div class="book-cover-sub">' + (lang === 'fr' ? 'Édition Française' : 'English Edition') + '</div>' +
        '</div>' +
      '</div>' +
      '<div class="book-meta">' +
...
function renderAuthorSheetBooks(books, lang) {
  ...
  list.innerHTML = books.map((b, i) => {
    ...
    const titleFr = b.title_fr;
    const title = lang === 'fr' ? titleFr : titleEn;
    const desc = lang === 'fr' ? b.description_fr : b.description_en;
    const editionLabel = lang === 'fr' ? 'ÉDITION FRANÇAISE · H.E. DR. TIDIANE OUATTARA' : 'ENGLISH EDITION · H.E. DR. TIDIANE OUATTARA';
    const isLast = i === books.length - 1;
    const borderStyle = isLast ? 'border-bottom:none;' : '';
    const coverSrc = lang === 'fr' && b.cover_image_fr ? b.cover_image_fr : b.cover_image;
    const coverClass = coverSrc ? 'sheet-book-cover book-cover-image' : 'sheet-book-cover bc-' + ((i % 3) + 1);
    const coverHtml = coverSrc
      ? '<img src="' + coverSrc + '" alt="' + title + '" loading="lazy" style="width:100%;height:auto;display:block;">'
      : '';
    return '<div class="sheet-book-detail" style="' + borderStyle + '">' +
      '<div class="' + coverClass + '" style="position:relative;overflow:hidden;">' +
        coverHtml +
        '<div style="position:absolute;inset:0;background:linear-gradient(to top,rgba(10,12,27,.85) 0%,transparent 60%)"></div>' +
        '<div style="position:absolute;bottom:20px;left:24px;right:24px;">' +
          '<div style="font-family:var(--ff-heading);font-size:1rem;font-weight:900;color:var(--white);line-height:1.2;" class="book-cover-light-text">' + title + '</div>' +
          '<div style="font-family:var(--ff-mono);font-size:.55rem;color:var(--gold-light);margin-top:6px;letter-spacing:.1em;">' + editionLabel + '</div>' +
        '</div>' +
      '</div>' +
      '<div>' +
        '<div class="book-status">' + b.status + '</div>' +
